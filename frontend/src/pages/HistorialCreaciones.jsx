...
const HistorialCreaciones = () => {
    ...
    if (generandoPDF && invitacionActiva && areaPdfRef.current) {
      const generarDocumento = async () => {
        try {
          ...
          setAviso({ visible: true, mensaje: "¡Invitación en PDF descargada con éxito!", tipo: 'exito' });

        } catch (error) {
          console.error("Error crítico al generar PDF:", error);
          setAviso({ visible: true, mensaje: "Hubo un problema al crear el archivo. Inténtalo de nuevo.", tipo: 'error' });
        } finally {
  ...
  const descargarQR = () => {
    ...
    if (canvas) {
      ...
      setAviso({ visible: true, mensaje: "¡Código QR descargado con éxito!", tipo: 'exito' });
    }
  };
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] w-full overflow-hidden flex flex-col items-center bg-sky-50">
      ...
      <div 
        ...
        style={{ backgroundImage: `url(${tieneCreaciones ? fondoMosaico : '/historial_vacio.png'})` }}
      >
          ...
          <div className="w-full max-w-6xl mx-auto flex flex-col">
              ...
              <div className="w-full bg-black/20 p-5 sm:p-8 md:px-12 md:py-6 flex flex-col md:flex-row justify-between items-center md:items-center gap-4 md:gap-6 text-center md:text-left">
                <div className="flex flex-col">
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tighter drop-shadow-sm">
                    Mis <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-pink-300">Creaciones</span>
                  </h1>
            ...
            {generandoPDF && (
              <div className="w-full mb-5 bg-sky-500/20 backdrop-blur-xl border border-sky-500/30 p-4 rounded-2xl shadow-sm flex justify-center items-center gap-3 animate-pulse">
                ...
                <p className="text-xs sm:text-sm text-sky-200 font-bold tracking-widest uppercase">Generando archivo PDF...</p>
              </div>
            )}

            {/* GRILLA DE INVITACIONES ADAPTATIVA (AQUÍ ESTÁ EL CAMBIO) */}
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 pb-4">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center w-full h-full min-h-[50vh] px-2 sm:px-4">
            <div className="bg-black/40 backdrop-blur-2xl p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border border-white/10 flex flex-col items-center shadow-2xl max-w-2xl w-full">
              ...
              <p className="text-xs sm:text-sm md:text-lg text-gray-200 font-medium mb-8 sm:mb-10 drop-shadow-md px-2 sm:px-4">
                Tu historial está vacío. Anímate a diseñar tu primera invitación y empieza a compartir momentos inolvidables.
              </p>
      ...
      {modalQR && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[9999] flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 md:p-10 max-w-sm w-full relative shadow-2xl">
            ...
            <p className="text-center text-gray-500 text-xs sm:text-sm font-medium mb-6 sm:mb-8 truncate px-4">{invitacionActiva?.titulo}</p>
            ...
            <div className="bg-sky-50 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] flex justify-center mb-6 sm:mb-8 border border-sky-100 shadow-inner">
              <QRCodeCanvas id="qr-canvas-historial" value={`${window.location.origin}/invitacion/${invitacionActiva?.id}`} size={180} lg:size={200} level={"H"} />
            </div>
      ...
      {invitacionActiva && generandoPDF && (
        <div style={{ position: "absolute", zIndex: "-100", top: "0", left: "0", opacity: 0, pointerEvents: "none", width: "100%" }}>
          <div ref={areaPdfRef} className="bg-white relative" style={{ width: '680px', margin: '0 auto', paddingBottom: '20px' }}>
             <RenderizadorPlantilla invitacion={invitacionActiva} urlImagen={urlImagenActiva} esModoPDF={true} />
          </div>
        </div>
      )}
    </div>
