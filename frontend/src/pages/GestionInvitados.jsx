...
const GestionInvitados = () => {
  ...
  // ESTADO PARA RASTREAR INVITACIONES ENVIADAS POR WHATSAPP
  const [enviadosWS, setEnviadosWS] = useState(() => {
    try {
      const data = localStorage.getItem(`ws_enviados_${id}`);
  ...
  const compartirEnlaceUnico = (invitado) => {
    ...
    // Al enviar, se guarda el ID del invitado como "enviado"
    if (!enviadosWS.includes(invitado.id)) {
      const nuevosEnviados = [...enviadosWS, invitado.id];
      setEnviadosWS(nuevosEnviados);
      localStorage.setItem(`ws_enviados_${id}`, JSON.stringify(nuevosEnviados));
    }
  };
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] w-full overflow-hidden flex flex-col items-center bg-sky-50">
      ...
      <ContenedorPrincipal className="relative z-10 w-full animate-fade-in-up py-5">
        ...
        <div className="bg-black/25 backdrop-blur-2xl rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden flex flex-col lg:flex-row items-stretch w-full max-w-6xl mx-auto shadow-2xl">
          ...
          <div className="w-full lg:w-7/12 flex flex-col bg-black/20">
            ...
            <div className="p-6 sm:p-8 md:p-12 border-b border-white/10 flex flex-col h-full min-h-[400px] md:min-h-[500px]">
              ...
              {/* --- MÉTRICAS DE ASISTENCIA RE-INTEGRADAS --- */}
              {invitados.length > 0 && (
                ...
                {invitados.length === 0 ? (
                  ...
                  <ul className="space-y-3 sm:space-y-4 max-h-[350px] sm:max-h-[450px] overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full">
                    {invitados.map((invitado) => {
                      ...
                      const enviado = enviadosWS.includes(invitado.id);

                      return (
                        <li key={invitado.id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 sm:p-5 rounded-[1.2rem] sm:rounded-[1.5rem] transition-all shadow-sm group ${
                          enviado 
                            ? 'bg-sky-900/10 border-2 border-sky-400/30 hover:bg-sky-900/20' 
                            : 'bg-white/5 border border-white/10 hover:bg-white/10'
                        }`}>
                          
                          <div className="mb-3 flex flex-col gap-1.5 w-full sm:w-auto mt-1 sm:mt-0">
                            <div className="flex flex-wrap items-end gap-2">
                              <p className="font-black text-white text-base sm:text-lg drop-shadow-sm">{invitado.nombre}</p>
                              {invitado.email && <span className="text-[10px] sm:text-xs text-gray-400 font-medium mb-1">{invitado.email}</span>}
                            </div>
                            
                            <div className="flex flex-wrap items-center gap-2 mt-1">
                              {renderEstado(invitado.estado)}
                              
                              {/* BADGE DE WHATSAPP ENVIADO O NO */}
                              {enviado ? (
                                <span className="bg-sky-500/20 border border-sky-500/30 text-sky-300 font-black px-3 py-1.5 rounded-full text-[8px] sm:text-[9px] uppercase tracking-widest shadow-sm flex items-center gap-1.5 w-fit">
                                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                                  Invitación Enviada
                                </span>
                              ) : (
                                <span className="bg-white/5 border border-white/10 text-gray-400 font-black px-3 py-1.5 rounded-full text-[8px] sm:text-[9px] uppercase tracking-widest shadow-sm flex items-center gap-1.5 w-fit">
                                  Sin Enviar
                                </span>
                              )}
                            </div>
                          ...
                          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto mt-2 sm:mt-0">
                            <button 
                              ...
                              className={`flex-1 sm:flex-none flex items-center justify-center px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest shadow-sm transition-all active:scale-95 ${
                                enviado 
                                  ? 'bg-transparent border border-sky-400/50 text-sky-400 hover:bg-sky-400 hover:text-white'
                                  : 'bg-[#25D366]/20 border border-[#25D366]/40 text-[#25D366] hover:bg-[#25D366] hover:text-white'
                              }`}
                              title={enviado ? "Reenviar enlace por WhatsApp" : "Enviar enlace único por WhatsApp"}
                            >
