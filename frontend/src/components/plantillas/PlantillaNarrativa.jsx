  ...
  return (
    <div className={`w-full mx-auto flex flex-col relative overflow-hidden ${esModoPDF ? 'pb-0 max-w-[700px]' : 'pb-10 sm:pb-20 max-w-4xl'}`}>
      ...
      <ComponenteAnimado {...animationProps} className="text-center px-4 relative z-10">
        <h1 className={`font-black mb-3 sm:mb-4 px-2 pb-2 sm:pb-3 leading-tight ${esModoPDF ? 'text-3xl text-[#252525]' : 'text-3xl sm:text-4xl md:text-5xl bg-gradient-to-r from-sky-500 via-pink-300 to-sky-500 bg-clip-text text-transparent'}`}>
          {invitacion.titulo}
        </h1>
        <p className={`${esModoPDF ? 'text-sm' : 'text-sm sm:text-lg md:text-xl'} text-gray-600 font-serif leading-relaxed sm:leading-loose max-w-3xl mx-auto whitespace-pre-wrap`}>
          {invitacion.mensaje}
        </p>
      </ComponenteAnimado>
      ...
      {/* 2. COLLAGE VISUAL */}
      {urlImagen && (
        <ComponenteAnimado {...animationProps} className="relative w-full flex justify-center items-center py-8 sm:py-12 md:py-16 my-2 sm:my-4 z-10">
          ...
          <div className={`absolute hidden sm:block w-32 h-32 md:w-56 md:h-56 bg-white p-1.5 sm:p-2 rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 z-0 left-2 sm:left-4 md:left-12 ${esModoPDF ? '' : '-rotate-12'} opacity-50`}>
          ...
          <div className={`absolute hidden sm:block w-32 h-32 md:w-56 md:h-56 bg-white p-1.5 sm:p-2 rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 z-0 right-2 sm:right-4 md:right-12 ${esModoPDF ? '' : 'rotate-12'} opacity-50`}>
            <div className="w-full h-full rounded-lg sm:rounded-xl grayscale" style={{ backgroundImage: `url(${urlImagen})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
          </div>
          <div className={`relative z-10 w-[85%] max-w-[280px] sm:max-w-none sm:w-56 sm:h-56 md:w-80 md:h-80 bg-white p-2.5 sm:p-3 md:p-4 rounded-[1.5rem] sm:rounded-3xl shadow-2xl border border-sky-50 ${esModoPDF ? '' : 'sm:rotate-2 sm:hover:rotate-0 transition-transform duration-500'}`}>
            <div className="w-full h-48 sm:h-full rounded-[1rem] sm:rounded-2xl" style={{ backgroundImage: `url(${urlImagen})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
          </div>
        </ComponenteAnimado>
      )}

      {/* 3. FECHA Y LUGAR */}
      <ComponenteAnimado {...animationProps} style={{ pageBreakInside: 'avoid' }} className="w-full mb-8 sm:mb-10 mt-4 sm:mt-6 relative z-10 px-2">
        <div className="flex flex-col md:flex-row bg-white rounded-[1.5rem] sm:rounded-[2rem] shadow-md border border-gray-100 overflow-hidden">
          <div className="w-full md:w-1/2 p-5 sm:p-6 md:p-8 bg-pink-50 border-b md:border-b-0 md:border-r border-pink-100 flex flex-col justify-center items-center sm:items-start text-center sm:text-left">
            ...
            <div className={`font-black text-[#252525] ${esModoPDF ? 'text-xl' : 'text-xl sm:text-2xl md:text-3xl'}`}>
              {new Date(invitacion.fecha_evento).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })}
            </div>
            {invitacion.hora_inicio && (
              <div className="mt-1 sm:mt-2 text-pink-500 font-bold text-base sm:text-lg md:text-xl">
                A las {invitacion.hora_inicio.slice(0, 5)} h
              </div>
            )}
          </div>
          <div className="w-full md:w-1/2 p-5 sm:p-6 md:p-8 bg-sky-50 flex flex-col justify-center items-center sm:items-start text-center sm:text-left">
      ...
      {entradasSinTimeline.length > 0 && (
        <ComponenteAnimado {...animationProps} style={{ pageBreakInside: 'avoid' }} className="px-2 w-full relative z-10">
          ...
          <div className={`flex items-start w-full ${esModoPDF ? 'flex-row gap-8' : 'flex-col sm:flex-row gap-4 sm:gap-6 md:gap-12'}`}>
            ...
            <div className={`flex flex-col gap-3 sm:gap-4 ${esModoPDF ? 'w-1/2' : 'w-full sm:w-1/2'}`}>
              {extraIzquierda.map(([clave, valor]) => (
                <div key={clave} className="flex items-start gap-3 sm:gap-4 bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-50 shadow-sm w-full">
      ...
      <ComponenteAnimado {...animationProps} style={{ pageBreakInside: 'avoid' }} className="text-center mt-10 sm:mt-12 flex flex-col items-center justify-center relative z-10 w-full">
