        ...
        {items.map(([clave, valor]) => (
          <div key={clave} className="flex items-start break-inside-avoid w-full">
            ...
            <div className="flex-1 w-full overflow-hidden">
              <span className="block text-[7px] sm:text-[9px] font-bold text-gray-400 uppercase tracking-wider">{clave}</span>
              <div className={`${esModoPDF ? 'text-xs' : 'text-[10px] sm:text-sm'} text-gray-800 font-serif font-medium mt-0.5 w-full`}>
  ...
  return (
    <div className={`w-full mx-auto flex flex-col relative overflow-hidden ${esModoPDF ? 'pb-0 max-w-[700px]' : 'pb-10 sm:pb-20 max-w-5xl px-2 sm:px-4'}`}>
      ...
      <div className={`relative z-10 flex ${esModoPDF ? 'flex-row gap-6' : 'flex-row gap-3 sm:gap-10'} items-start w-full`}>
        ...
        <div className={`flex flex-col ${esModoPDF ? 'w-1/2 gap-6' : 'w-1/2 gap-4 sm:gap-8'}`}>
          ...
          <ComponenteAnimado {...animationProps} className="text-left px-1 sm:px-0">
            ...
            <p className={`${esModoPDF ? 'text-sm' : 'text-xs sm:text-base md:text-lg'} text-gray-700 font-serif leading-relaxed sm:leading-loose whitespace-pre-wrap break-words`}>
              {invitacion.mensaje}
            </p>
          </ComponenteAnimado>
            ...
            <ComponenteAnimado {...animationProps} className="w-full shadow-md rounded-[1rem] sm:rounded-[2rem]">
              {esModoPDF ? (
                ...
                <img 
                  src={urlImagen} 
                  alt="Portada" 
                  className="w-full h-32 sm:h-64 md:h-[300px] object-cover rounded-[1rem] sm:rounded-[2rem] bg-white" 
                  crossOrigin="anonymous" 
                />
              )}
            </ComponenteAnimado>
          ...
          <ComponenteAnimado {...animationProps} style={{ pageBreakInside: 'avoid' }} className="flex flex-col gap-3 sm:gap-4">
            <div className="bg-gradient-to-br from-sky-400 to-sky-600 p-3 sm:p-5 rounded-[1rem] sm:rounded-[1.5rem] shadow-md text-white flex flex-col items-center justify-center text-center">
              ...
              {invitacion.hora_inicio && (
                <div className={`font-black ${esModoPDF ? 'mt-1 text-2xl' : 'mt-1 text-xl sm:text-3xl'}`}>
                  {invitacion.hora_inicio.slice(0, 5)} <span className={`${esModoPDF ? 'text-sm' : 'text-xs sm:text-lg'} font-normal opacity-80`}>h</span>
                </div>
              )}
            </div>
            ...
            <div className="bg-white p-3 sm:p-5 rounded-[1rem] sm:rounded-[1.5rem] shadow-md border border-sky-100 flex flex-col items-center justify-center text-center">
              ...
              <div className="mt-1.5 sm:mt-3 px-2 sm:px-3 py-1 sm:py-1.5 bg-sky-50 text-sky-500 rounded-full text-[7px] sm:text-[10px] font-bold border border-sky-100 inline-block leading-none uppercase tracking-wider">
                Punto de encuentro
              </div>
            </div>
          </ComponenteAnimado>
      ...
      >
        ...
        <img src="/logo_CWM_largo.png" alt="Come With Me" className="w-24 sm:w-48 opacity-70 object-contain grayscale" />
      </ComponenteAnimado>
