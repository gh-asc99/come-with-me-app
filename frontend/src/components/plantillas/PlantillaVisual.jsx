...
const PlantillaVisual = ({ invitacion, urlImagen, esModoPDF = false }) => {
    ...
    if (Array.isArray(valor)) {
      if (valor.length > 0 && valor[0].hora !== undefined) {
        return (
          <div className="mt-3 w-full flex flex-col gap-2.5 font-sans">
            {valor.map((fase, i) => (
              <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 bg-white/50 p-2.5 rounded-xl border border-gray-100/50 shadow-[0_2px_10px_rgba(0,0,0,0.02)] break-inside-avoid">
      ...
      return (
        <div className="mt-2 w-full flex flex-wrap gap-2 font-sans">
          {valor.map((item, i) => (
            <span key={i} className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-[9px] sm:text-[10px] md:text-xs font-bold border bg-white shadow-sm ${colorText} ${colorBorder}`}>
              {item}
            </span>
          ))}
        </div>
  ...
  return (
    <div className={`w-full mx-auto flex flex-col relative ${esModoPDF ? 'pb-0 max-w-[700px]' : 'pb-10 sm:pb-20 max-w-4xl'}`}>
        ...
        <div className={`relative ${esModoPDF ? 'w-[45%]' : 'w-full sm:w-[45%] max-w-[280px] sm:max-w-none'} flex justify-center`}>
          <ComponenteAnimado {...animationProps} className="relative w-full">
            ...
            {urlImagen ? (
              esModoPDF ? (
                <div 
                  className="w-full h-[280px] rounded-[1.5rem] sm:rounded-3xl bg-white shadow-lg border-2 border-white"
                  style={{ backgroundImage: `url(${urlImagen})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}
                />
              ) : (
      ...
      <ComponenteAnimado {...animationProps} style={{ pageBreakInside: 'avoid' }} className={`grid ${esModoPDF ? 'grid-cols-2 gap-6' : 'grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6'} px-2 relative z-10 w-full`}>
        <div className="bg-white p-5 sm:p-6 md:p-8 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border-2 border-pink-100 flex flex-col items-center justify-center text-center relative overflow-hidden">
          ...
          {invitacion.hora_inicio && (
            <div className={`font-black text-pink-500 ${esModoPDF ? 'mt-1 text-3xl' : 'mt-1 sm:mt-2 text-3xl sm:text-4xl'}`}>
          ...
          <div className={`flex items-start w-full ${esModoPDF ? 'flex-row gap-8' : 'flex-col sm:flex-row gap-6 sm:gap-8 md:gap-12'}`}>
            ...
            <div className={`flex flex-col gap-5 sm:gap-6 ${esModoPDF ? 'w-1/2' : 'w-full sm:w-1/2'}`}>
              {extraIzquierda.map(([clave, valor]) => (
                <div key={clave} className="flex items-start gap-3 sm:gap-4 break-inside-avoid w-full">
