  ...
  const renderizarValor = (valor, indice) => {
    ...
    if (esUrlImagen(valor)) {
      if (esModoPDF) {
        return (
          <div 
            className={`mt-4 w-full rounded-xl shadow-sm border border-${colorPrimario}-100 flex-shrink-0 relative z-10`}
            style={{
              height: '160px',
      ...
      return (
        <div className={`mt-4 w-full rounded-xl overflow-hidden shadow-sm border border-${colorPrimario}-100 relative z-10`}>
    ...
    if (Array.isArray(valor)) {
      if (valor.length > 0 && valor[0].hora !== undefined) {
        return (
          <ul className={`mt-4 border-l-2 border-${colorPrimario}-200 pl-4 space-y-4 font-sans w-full relative z-10`}>
            {valor.map((fase, i) => (
      ...
      return (
        <ul className={`mt-3 space-y-2 font-sans text-xs sm:text-sm text-gray-600 relative z-10`}>
          {valor.map((item, i) => (
    ...
    return <span className="font-serif leading-relaxed sm:leading-snug text-sm sm:text-base text-gray-700 mt-2 block relative z-10">{valor}</span>;
  };

  return (
    <div className={`w-full mx-auto flex flex-col relative bg-[#fafafa] ${esModoPDF ? 'gap-8 pb-0 max-w-[700px] min-h-screen overflow-visible' : 'gap-8 sm:gap-10 pb-10 sm:pb-20 max-w-4xl shadow-2xl overflow-hidden'}`}>

      {/* MARCA DE AGUA GIGANTE (SOLO PDF Y USUARIOS TIPO USER) */}
      ...
      {urlImagen && (
        <ComponenteAnimado {...animationProps} className="relative w-full flex justify-center z-10 px-4 sm:px-6">
          {esModoPDF ? (
            <div 
              className="w-full rounded-[1.5rem] bg-white shadow-lg relative z-10"
              style={{
                height: '250px',
            ...
            <img 
              src={urlImagen} 
              alt="Portada" 
              className="w-full sm:max-w-full max-h-[250px] sm:max-h-[380px] object-cover sm:object-contain rounded-[1.5rem] sm:rounded-[2rem] shadow-xl bg-white border border-gray-100 relative z-10" 
      ...
      {/* 2. CABECERA */}
      <ComponenteAnimado {...animationProps} className="text-center px-6 sm:px-8 relative z-10 mt-2 sm:mt-0 bg-white/40 backdrop-blur-sm py-4 rounded-2xl mx-4 sm:mx-6">
      ...
      {/* 4. SUGERENCIAS (CON FONDO MOSAICO) */}
      {invitacion.datos_extra && Object.keys(invitacion.datos_extra).length > 0 && (
        ...
        >
          ...
          <div className="relative px-6 sm:px-8 pt-8 pb-8 sm:pb-10 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-lg border border-gray-200 overflow-hidden bg-white">
            ...
            <div className="text-center mb-8 sm:mb-10 relative z-10" style={esModoPDF ? { pageBreakInside: 'avoid' } : {}}>
              <span className="inline-block bg-white/90 backdrop-blur-sm px-6 py-2 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-gray-800 shadow-md border border-gray-100">
                ...
                {extraIzquierda.map(([clave, valor], idx) => {
                  ...
                  return (
                    <div key={clave} className="break-inside-avoid" style={esModoPDF ? { pageBreakInside: 'avoid' } : {}}>
                      <div className="bg-white/90 backdrop-blur-md p-5 rounded-2xl border border-gray-100 shadow-md relative overflow-hidden group hover:border-gray-200 transition-colors duration-300">
      ...
      <ComponenteAnimado 
        ...
        className="text-center mt-8 sm:mt-12 flex flex-col items-center justify-center relative z-10 w-full mb-6 bg-white/40 backdrop-blur-sm py-4 rounded-xl mx-4 sm:mx-6"
      >
        <p className="text-gray-500 font-serif italic text-xs sm:text-sm mb-3 sm:mb-4">
          Un evento diseñado por <span className="font-bold text-gray-700 not-italic">{nombreCreador}</span>
        </p>
        <img 
